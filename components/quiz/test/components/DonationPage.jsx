...
const DonationPage = ({ setPage, user, onLogout, setShowAuth, onCreate }) => {
    const [selectedAmount, setSelectedAmount] = useState(null);
    const [customAmount, setCustomAmount] = useState('');
    const [isProcessing, setIsProcessing] = useState(false);
    const [paypalUrl, setPaypalUrl] = useState('https://paypal.me/keishoinc'); // PayPal.Meリンク
    const [donationStatus, setDonationStatus] = useState(null); // 'success' | 'cancel' | null

    // ページタイトル設定とURLパラメータから決済結果を確認
    useEffect(() => {
        // ブラウザタブのタイトルを設定
        document.title = "応援ありがとうございます | 診断クイズメーカー";
        window.scrollTo(0, 0);
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-rose-50 via-white to-amber-50 font-sans flex flex-col">
                ...
                <div className="max-w-3xl mx-auto px-4 py-12">
                    ...
                    <div className="bg-white rounded-2xl shadow-lg p-6 sm:p-8 mb-8 border border-rose-100">
                        ...
                        <div className="space-y-4 text-gray-700 leading-relaxed">
                            <p>
                                診断クイズメーカーは、<span className="font-bold text-rose-600">誰でも無料で</span>診断コンテンツを作成できるサービスとして運営しています。
                            </p>
                            <p>
            ...
            <Footer setPage={setPage} onCreate={onCreate} user={user} setShowAuth={setShowAuth} />
        </div>
    );
};
