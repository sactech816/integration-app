    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-5 p-6 border rounded-lg shadow-sm bg-white">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <label className="flex flex-col">
          ...
          <input name="name" type="text" required className="border p-2 rounded" placeholder="山田 太郎" />
        </label>
        <label className="flex flex-col">
          <span className="font-bold text-sm mb-1">メールアドレス</span>
          <input name="email" type="email" required className="border p-2 rounded" placeholder="example@gmail.com" />
        </label>
      </div>

      <label className="flex flex-col">
        <span className="font-bold text-sm mb-1">件名</span>
        <input name="subject" type="text" required className="border p-2 rounded" placeholder="お仕事の依頼について" />
      </label>

      <label className="flex flex-col">
        <span className="font-bold text-sm mb-1">メッセージ</span>
        <textarea name="message" rows={5} required className="border p-2 rounded" placeholder="詳細をご記入ください" />
      </label>
