...
import { FileText, Star, Compass, Sparkles, Loader2, Download, Crown, CheckCircle, Heart } from 'lucide-react';
...
export default function FortunePremiumReport({
  ...
  const handlePrint = useCallback(() => {
    ...
    if (printWindow && reportHtml) {
      printWindow.document.write(`
        ...
        <head>
          <meta charset="utf-8">
          <title>生年月日占い プレミアム鑑定レポート</title>
  ...
  const handleDownloadPdf = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch('/api/fortune/download-pdf', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ resultId }),
      });
        ...
        <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="flex items-center gap-1 px-3 py-1.5 text-sm font-medium text-indigo-700 hover:text-indigo-800 bg-indigo-50 border border-indigo-200 rounded-lg transition-all disabled:opacity-50"
            >
              {downloadingPdf ? (
