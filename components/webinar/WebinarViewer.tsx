'use client';

import React from 'react';
import { WebinarLP } from '@/lib/types';
import { ViewTracker, trackClick } from '@/components/shared/ViewTracker';
import ContentFooter from '@/components/shared/ContentFooter';
import TrackingScripts from '@/components/shared/TrackingScripts';
import BlockRenderer from '@/components/shared/BlockRenderer';

interface WebinarViewerProps {
  lp: WebinarLP;
}

const WebinarViewer: React.FC<WebinarViewerProps> = ({ lp }) => {
  // リンククリックハンドラー（slugを使用してアナリティクスと統一）
  const handleLinkClick = (url: string) => {
    if (lp.slug) {
      trackClick(lp.slug, 'webinar', url);
    }
  };

  // テーマ設定から背景スタイルを生成
  const theme = lp.settings?.theme;
  const backgroundImage = theme?.backgroundImage;
  const gradient = theme?.gradient;
  const isAnimated = theme?.animated !== false; // デフォルトはアニメーション有効

  // 単色かグラデーションかを判定（#で始まる場合は単色）
  const isSolidColor = gradient?.startsWith('#');

  const backgroundStyle: React.CSSProperties = backgroundImage
    ? {
        backgroundImage: `linear-gradient(rgba(0, 0, 0, 0.3), rgba(0, 0, 0, 0.3)), url(${backgroundImage})`,
        backgroundSize: 'cover',
        backgroundPosition: 'center',
        backgroundAttachment: 'fixed',
      }
    : gradient && isSolidColor
    ? {
        backgroundColor: gradient,
      }
    : gradient
    ? {
        background: gradient,
        backgroundSize: isAnimated ? '400% 400%' : 'auto',
      }
    : {};

  // グラデーションアニメーションのクラス（単色の場合はアニメーションなし）
  const animationClass = gradient && !backgroundImage && !isSolidColor && isAnimated ? 'animate-gradient-xy' : '';

  return (
    <>
      {/* アナリティクストラッカー（slugを使用）*/}
      {lp.slug && (
        <ViewTracker
          contentId={lp.slug}
          contentType="webinar"
          trackScroll={true}
          trackTime={true}
        />
      )}

      {/* 外部計測タグ */}
      <TrackingScripts settings={lp.settings?.tracking} />

      <div
        className={`min-h-screen ${!backgroundImage && !gradient ? 'bg-gray-900' : ''} ${animationClass}`}
        style={backgroundStyle}
      >
        {lp.content?.map(block => (
          <BlockRenderer
            key={block.id}
            block={block}
            variant="business"
            onLinkClick={handleLinkClick}
          />
        ))}

        {/* フッター */}
        <ContentFooter toolType="webinar" variant="dark" hideFooter={lp.settings?.hideFooter} />
      </div>
    </>
  );
};

export default WebinarViewer;
