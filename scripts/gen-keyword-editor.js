...
import {
  ...
  AlertCircle, Loader2, BarChart3, ArrowLeft, Users, TrendingUp, AlertTriangle, Download,
} from 'lucide-react';
...
function exportToCSV(data: YouTubeVideoData[], keyword: string) {
  const BOM = '\\uFEFF';
  const headers = [
    '順位',
    'タイトル',
  ...
  const rows = data.map((d, i) => {
    const engRate = d.viewCount > 0 ? (((d.likeCount + d.commentCount) / d.viewCount) * 100).toFixed(3) : '0';
    const likeRate = d.viewCount > 0 ? ((d.likeCount / d.viewCount) * 100).toFixed(3) : '0';
    return [
      i + 1,
      '"' + d.title.replace(/"/g, '""') + '"',
      '"' + d.channelTitle.replace(/"/g, '""') + '"',
      d.viewCount,
      d.subscriberCount,
      d.viewRatio,
      d.likeCount,
  ...
  a.href = url;
  a.download = 'youtube_keyword_' + keyword.replace(/\\s+/g, '_') + '_' + new Date().toISOString().slice(0, 10) + '.csv';
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="sticky top-16 z-40 bg-white border-b border-gray-200 shadow-sm">
        <div className="max-w-[1920px] mx-auto px-4 py-3 flex items-center justify-between">
          ...
          {results.length > 0 && (
            <button
              onClick={() => exportToCSV(sortedResults, searchedKeyword)}
              className="flex items-center gap-2 px-4 py-2 bg-green-50 border border-green-200 text-green-700 font-semibold rounded-xl hover:bg-green-100 transition-all text-sm min-h-[44px] shadow-sm"
            >
...
const targetPath = path.join(__dirname, '..', 'components', 'youtube-keyword-research', 'YouTubeKeywordResearchEditor.tsx');
fs.mkdirSync(path.dirname(targetPath), { recursive: true });
fs.writeFileSync(targetPath, content, 'utf8');
console.log('Written', content.length, 'chars to', targetPath);
