...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!data) return { title: 'スロットが見つかりません' };

  return generateUGCMetadata({
    title: data.title,
    description: data.description || `${data.title} - デジタルスロット`,
    type: 'slot',
    slug: campaign_id,
  });
}
...
export default async function SlotPage({ params }: Props) {
  ...
  const schema = generateUGCSchema({
    ...
    name: title,
    description: description || `${title} - デジタルスロット`,
    url: `${siteUrl}/slot/${campaign_id}`,
  });
