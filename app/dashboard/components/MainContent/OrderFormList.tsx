...
import {
  ...
  CreditCard, Eye, FileText, BarChart3, Link2, BookOpen,
  Settings, Layout, ListPlus, Send,
} from 'lucide-react';
...
export default function OrderFormList({
  ...
  const fetchForms = async () => {
    setLoading(true);
    const res = await fetch(`/api/order-form?userId=${userId}`);
    if (res.ok) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1 text-sm">フォーム作成・申し込み管理</p>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => setShowOnboarding(true)}
            className="inline-flex items-center gap-1.5 px-3 py-2 text-sm text-gray-600 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors min-h-[44px]"
          >
          ...
          <Link
            href="/order-form/new"
            className="inline-flex items-center gap-2 px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold rounded-xl shadow-md hover:shadow-lg transition-all min-h-[44px]"
          >
      ...
      {!loading && forms.length === 0 && (
        <div className="bg-white rounded-2xl border border-gray-200 p-8 text-center shadow-sm">
          ...
          <Link
            href="/order-form/new"
            className="inline-flex items-center gap-2 px-6 py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold rounded-xl shadow-md transition-all min-h-[44px]"
          >
