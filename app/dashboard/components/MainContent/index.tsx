...
import ConciergeOperatorView from '@/app/dashboard/components/Admin/ConciergeOperator';
...
type MainContentProps = {
  ...
  adminComponents?: {
    ...
    FeatureProductManager?: () => React.ReactNode;
    PurchaseHistoryManager?: () => React.ReactNode;
  };
};
...
export default function MainContent({
  ...
  useEffect(() => {
    ...
    <div ref={contentRef}>
      ...
      {['quiz', 'entertainment', 'profile', 'business', 'salesletter', 'onboarding', 'thumbnail', 'webinar', 'sns-post', 'site', 'swipe'].includes(activeView) && (
      ...
      {activeView === 'admin-products' && adminComponents?.FeatureProductManager && (
        <>{adminComponents.FeatureProductManager()}</>
      )}

      {activeView === 'admin-purchases' && adminComponents?.PurchaseHistoryManager && (
        <>{adminComponents.PurchaseHistoryManager()}</>
      )}

      {/* コンシェルジュ チャット対応（オペレーターコンソール） */}
      {activeView === 'admin-concierge-operator' && <ConciergeOperatorView />}
