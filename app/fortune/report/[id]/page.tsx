...
export default function FortuneReportPage() {
  ...
  const loadReport = async () => {
    try {
      ...
      setBirthLabel(`${result.birth_year}年${result.birth_month}月${result.birth_day}日生まれ`);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-white">
        <div className="text-center max-w-md px-4">
          ...
          <button
            onClick={() => router.push(`/fortune/result/${id}`)}
    ...
    <>
      ...
      <div className="no-print sticky top-0 z-50 bg-white border-b border-gray-200 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <span className="text-sm text-gray-500 hidden sm:block">{birthLabel}</span>
            <button
              onClick={handlePrint}
              className="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-lg font-medium text-sm shadow-sm hover:bg-indigo-700 transition-all"
            >
              <Printer size={16} />
              PDF保存 / 印刷
            </button>
      ...
      <div className="report-container">
        ...
        <div className="print-only-footer">
          <p>生年月日占い プレミアム鑑定レポート | {birthLabel}</p>
