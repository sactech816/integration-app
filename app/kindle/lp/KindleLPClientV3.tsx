...
export default function KindleLPClient() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-amber-50 to-orange-50">
      ...
      {/* シンプルヘッダー（提供されたLP内容に基づく） */}
      <header className="fixed top-0 left-0 right-0 bg-white/95 backdrop-blur-sm shadow-sm z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <Link href="/" className="flex items-center gap-2">
              <span className="text-2xl font-bold text-blue-600">集客メーカー</span>
            </Link>
            <nav className="hidden md:flex items-center gap-6 text-sm font-medium">
              <a href="#demo" className="text-gray-600 hover:text-blue-600 transition">Kindle出版デモ一覧</a>
              <a href="/works" className="text-gray-600 hover:text-blue-600 transition">作品集</a>
              <Link href="/news" className="text-gray-600 hover:text-blue-600 transition">お知らせ</Link>
              {user ? (
                <Link href="/kindle" className="bg-blue-600 text-white px-6 py-2 rounded-full hover:bg-blue-700 transition">
                  マイページ
                </Link>
              ) : (
                <button 
            ...
            <button 
              onClick={() => setShowAuth(true)}
              className="md:hidden bg-blue-600 text-white px-4 py-2 rounded-full text-sm hover:bg-blue-700 transition"
            >
              {user ? 'マイページ' : 'ログイン'}
            </button>
          </div>
      ...
      {showScrollTop && (
        <button
          onClick={scrollToTop}
          className="fixed bottom-6 right-6 bg-blue-600 hover:bg-blue-700 text-white p-3 rounded-full shadow-2xl transition-all transform hover:scale-110 z-50"
