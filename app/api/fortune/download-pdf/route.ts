/**
 * Fortune プレミアムレポート — レポートページへリダイレクト
 * GET  /api/fortune/download-pdf?id=<resultId>
 * POST /api/fortune/download-pdf { resultId }
 *
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ url: `${SITE_URL}/fortune/report/${id}` });
  } catch (err: any) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ url: `${SITE_URL}/fortune/report/${resultId}` });
  } catch (err: any) {
